def main():
    ...
    if vin in vehicles_dict:
        ...
        list_cars = vehicles_dict[vin]

        # Print the manufacturer, model, and color of the vehicle.
        # Don't print the year, engine design, or displacement.
        manufacturer = list_cars[MANUFACTURER_INDEX]
        model = list_cars[MODEL_INDEX]
        color = list_cars[COLOR_INDEX]

        print (f"Manufaturer: {manufacturer} - Model: {model} - Color: {color}")
